...
class ItemForm(FlaskForm):
    ...
    id       = StringField("ID")
    date       = StringField("DATE", validators=[InputRequired("Input is required!"), DataRequired("Data is required!"), Length(min=5, max=20, message="Input must be between 5 and 20 characters long")])
    pod_no       = StringField("AWB  Number", validators=[InputRequired("Input is required!"), DataRequired("Data is required!"), Length(min=5, max=20, message="Input must be between 5 and 20 characters long")])
    sname       = StringField("Sender Name", validators=[InputRequired("Input is required!"), DataRequired("Data is required!"), Length(min=5, max=20, message="Input must be between 5 and 20 characters long")])
    rname       = StringField("Reciver Name ", validators=[InputRequired("Input is required!"), DataRequired("Data is required!"), Length(min=5, max=20, message="Input must be between 5 and 20 characters long")])
    scontact       = PriceField("Sender Contact No.")
    rcontact       = PriceField("Reciver Contact No.")
...
def home():
    ...
    if form.validate():
        ...
        app.logger.info("coming")

        filter_queries = []
        parameters = []

        if form.pod_no.data.strip():
            print(form.pod_no.data.strip())
            filter_queries.append("i.booking_date LIKE ?")
            parameters.append("%" + escape(form.pod_notle.data) + "%")
    ...
    return render_template("homecopy.html", items=items, form=form)
...
def new_booking():
    ...
    if form.validate_on_submit():
        ...
        c.execute("""INSERT INTO booking
                    (booking_date, pod_no, sname, rname, scontact, rcontact,pincode,location,amount)
                    VALUES(?,?,?,?,?,?,?,?,?)""",
                    (
                        escape(form.date.data),
                        escape(form.pod_no.data),
                        escape(form.sname.data),
                        escape(form.rname.data),
                        float(form.scontact.data),
                        float(form.rcontact.data),
                        float(form.pincode.data),
                        escape(form.location.data),
        ...
        return redirect(url_for("home"))
    format = "%d-%m-%Y"
    form.date.data = datetime.datetime.utcnow().strftime(format)
